...
const Nfts: React.FC = () => {
    ...
    const infts_data = useQuery(inftsQuery({where: search,owner_: getStore.account[0] ?? '0x' }))
    ...
    useEffect( ()=>{
        ...
        // console.log('infts_data',infts_data)
        if (infts_data.data) {
            // console.log('infts_data1',infts_data.data.infts)
            setData({...data,list: infts_data.data.infts})
        } else {
            setData({...data,list: []})
        }
    },[infts_data,getStore.account,getStore.chainId,getStore.popupCoin])
    useEffect( ()=>{
        if (!data.search) {
            setSearch(data.search)
        }
                            ...
                            return (
                                <ul key={index}>
                                    ...
                                    <li className={'li3 li_type'}>
                                        <p className={'fz0'}>
                                            <span className={'span1 dib'}>{t('h_item94')}</span>
                                            <span className={'span2 dib'}>{getStore.chainName.toUpperCase()}</span>
                                        </p>
                                        <p className={'fz0'}>
                                            <span className={'span1 dib'}>{t('h_item144')}</span>
                                            <span className={'span2 dib'}>
                                                {item.nftType_ == '0' ? t('h_item145') : ''}
                                                {item.nftType_ == '1' ? t('h_item72') : ''}
                                                {item.nftType_ == '2' ? t('h_item146') : ''}
                                            </span>
                                        </p>
                                        <p className={'fz0'}>
                                            <span className={'span1 dib'}>{t('h_item147')}</span>
                                            <span className={'span2 dib'}>{item.maxMintNum_}</span>
                                        </p>
                                    </li>
                                    <li className={'li4 li_type2'}>
                                        <p className={'fz0'}><span className={'span1 dib'}>{t('h_item148')}</span><span className={'span2 dib'}>{BigNumberSub(item.maxMintNum_,item.mintedNum_)}</span></p>
                                        <p className={'fz0'}><span className={'span1 dib'}>{t('h_item151')}</span><span className={'span2 dib'}>{substringStr(item.contractAddress_)} <Copy toCopy={item.contractAddress_}/></span><span className={'color_green'}>{item.status=='1' ? `[${t('h_item149')}]` : ''}</span></p>
                                    </li>
                                    <li className={'li5 li_edit fz0'}>
                                        {
                                            item.nftType_ == '0' ?
                                                <button className={'b_edit dib'} onClick={()=>{setStore('popupCoin',true);setStore('popupInfo',item)}}>
