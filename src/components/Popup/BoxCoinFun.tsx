...
import {useSetBoxNftMint} from "../../tools/chain";
import * as metaMaskTools from "../../tools/chain";

export default function BoxCoinFun() {
    ...
    const { setBoxNftMint } = useSetBoxNftMint()
    const setCoinFun = useCallback(async ()=>{
        ...
        setBoxNftMint(data.NFTAddress,data.to,data.coinNum).then((res)=>{
            setStore('popupBoxCoin',false)
        })
    })
    useEffect(()=>{
    ...
    return (
        <>
            <Modal
                isOpen={getStore.popupBoxCoin}
                onDismiss={()=>{setStore('popupBoxCoin',false)}}
                width="440px"
                showHeader={true}
                title={t('h_item53')}
            >
